...
class Myorder extends MY_Controller {
    ...
    public function confirm($invoice)
    {
        ...
        if($data['order']->status !== 'waiting'){
            $this->session->set_flashdata('warning', 'Bukti transfer telah dikirim');
    ...
    public function image_required(){
        ...
        if(empty($_FILES) || $_FILES['image']['name'] === ''){
            $this->session->flashdata('image_error', 'Bukti transfer tidak boleh kosong');
            return false;
        }
        return true;
    }
